...
export const Route = createFileRoute("/simulators/env")({
  head: () => ({ meta: [{ title: "AI Lab → Prod Simulator" }, { name: "description", content: "Pick the right environment for each scenario." }] }),
  component: Env,
});
...
const cases = [
  ...
  { id: "c3", text: "Integration + security + connector tests with sanitized data.", ideal: "test" },
  ...
  { id: "c6", text: "Full rollout with monitoring, incident response, and recertification.", ideal: "prod" },
];

const stages = ["ai-lab","dev","test","uat","pilot","prod"] as const;
const stageLabel: Record<(typeof stages)[number], string> = { "ai-lab":"AI Lab", dev:"Dev", test:"Test", uat:"UAT", pilot:"Pilot", prod:"Production" };
...
function Env() {
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <PageHeader title="AI Lab → Prod Simulator" subtitle="Which environment does each scenario belong in today?" />
        ...
        {cases.map((c, i) => (
          <Card key={c.id}>
            ...
            <CardContent>
              ...
              {checked ? <p className="pt-2 text-xs text-muted-foreground">Ideal: <Badge variant="outline">{stageLabel[c.ideal as (typeof stages)[number]]}</Badge></p> : null}
            </CardContent>
          </Card>
        ))}
